...
{
    return (sizeof(ISTACK) + sizeof(PNG_DATA) * size);
}
...
int init_shm_stack(ISTACK *p, int stack_size)
{
    ...
    p->size = stack_size;
    p->pos  = -1;
    p->items = (PNG_DATA *) (p + sizeof(ISTACK));
    return 0;
}
...
ISTACK *create_stack(int size)
{
    ...
    if ( pstack == NULL ) {
        ...
        char *p = (char *)pstack;
        pstack->items = (PNG_DATA *) (p + sizeof(ISTACK));
        pstack->size = size;
        pstack->pos  = -1;
    }
...
int push(ISTACK *p, PNG_DATA item)
{
    if ( p == NULL ) {
        return -1;
    }
    if ( !is_full(p) ) {
		...
		fflush(stdout);
		int shmid_push_name = shmget(IPC_PRIVATE, 20*sizeof(char), IPC_CREAT | IPC_EXCL | S_IRUSR | S_IWUSR);
		int shmid_push_data = shmget(IPC_PRIVATE, 10000, IPC_CREAT | IPC_EXCL | S_IRUSR | S_IWUSR);
		printf("ASSIGNED MEM\n");
		fflush(stdout);
		p->items[p->pos].png_name = (char*)shmat(shmid_push_name, NULL, 0);
        p->items[p->pos].png = (RECV_BUF *)shmat(shmid_push_data, NULL, 0);
		printf("BEFORE ++\n");
		fflush(stdout);
		++(p->pos);
		memcpy(&p->items[p->pos].png, &item.png, 10000);
		memcpy(&p->items[p->pos].png_name, &item.png_name, 20*sizeof(char));
        /*p->items[p->pos] = item;*/
        return 0;
    } else {
        return -1;
...
int pop(ISTACK *p, PNG_DATA *p_item)
{
	if ( p == NULL ) {
        return -1;
    }
