    ...
    if (result) {
        ...
        for (i=0; i < nodeset->nodeNr; i++) {
            ...
            if ( href != NULL && !strncmp((const char *)href, "http", 4) ) {
				...
				//printf("adding url: %s\n", href);
				strcpy(visited_urls[vu_size], href);
				vu_size++;
...
static void init(CURLM *cm, int i)
{
  ...
  curl_easy_setopt(eh, CURLOPT_WRITEDATA, visited_bufs[i]);
  curl_easy_setopt(eh, CURLOPT_HEADER, 0L);
  curl_easy_setopt(eh, CURLOPT_URL, visited_urls[i]);
  curl_easy_setopt(eh, CURLOPT_PRIVATE, visited_urls[i]);
  ...
  //printf("visited URL %d: %s\n", i, visited_urls[i]);
}

int main( int argc, char** argv ) 
{
	for (int i = 0; i < 500; i++) {
		visited_bufs[i] = malloc(BUF_SIZE);
		memset(visited_bufs[i], 0, BUF_SIZE);
	}
	...
	int still_running = min(t, vu_size - vu_ind); // should be min between max # of concurrent connections and the # of urls we have stored to check in the frontier

	int orig = vu_ind;
	for (i = 0; i < still_running; ++i) {
        init(cm, vu_ind);
		vu_ind++;
    }
	...
	while ((msg = curl_multi_info_read(cm, &msgs_left))) {
        if (msg->msg == CURLMSG_DONE) {
			...
			//curl_easy_setopt(eh, CURLOPT_WRITEFUNCTION, write_cb_curl3);
			//curl_easy_setopt(eh, CURLOPT_WRITEDATA, &recv_buf);
			...
			int ind = 0;
			for (int i = 0; i < vu_size; i++) {
				if (strcmp(visited_urls[i], szUrl) == 0) {
					ind = i;
					break;
				}
			}
			...
			//printf("processing: %d\n", ind);
			process_data(eh, visited_bufs[ind]);

			//printf("Recv_buf = %d\n", visited_bufs[ind]->size);
            ...
            if(http_status_code==200) {
				...
				if (unique) {
                //printf("200 OK for %s\n", szUrl);
				strcpy(visited_urls[vu_size], szUrl);
				vu_size++;
				}
            } else {
			...
			if (png_count >= m) {
				while ((msg = curl_multi_info_read(cm, &msgs_left))) {
					//printf("inside ending while loop\n");
					//fflush(stdout);
        			if (msg->msg == CURLMSG_DONE) {
            			eh = msg->easy_handle;
						curl_multi_remove_handle(cm, eh);
